...
| `Step(float dt)` | `void` | Advances the simulation by `dt` seconds. No-op if `dt <= 0`. |

`material` defaults to `Material.Default` in both factory helpers.

## The Step pipeline

`Step` runs: apply force generators → integrate forces → broad phase → narrow phase →
velocity solver (×`VelocityIterations`) → integrate velocities → positional correction
(×`PositionIterations`) → clear force accumulators → raise `CollisionsResolved`. See
